...
function PostItem (props) {
  ...
  if (props.content.match(IMAGE_EXTENSIONS)) {
    return (
      <div>
        <img
          src={getAvatarFromSubject(props.maker)}
          style={{
            margin: '1px',
            borderRadius: '50%',
            height: '40px',
        ...
        <sup style={{ color: '#ADB2BB', verticalAlign: 'top' }}>
          <a style={{ fontWeight: 'bold' }} href={props.maker} target='_blank'>
            {getNameFromSubject(props.maker)}
          </a>{' '}
          &nbsp;
          {moment.utc(props.created).fromNow()} &nbsp;{' '}
          <a target='_blank' href={props.subject}>
            <img height='10' width='10' src='./image/External.svg' />
          </a>{' '}
        </sup>
        <br />
        <span style={{ fontFamily: 'Helvetica', fontSize: '14px' }}>
          {' '}
    ...
    return (
      <div>
        {props.id + 1}.{' '}
        <video controls autoplay='true' loop src={props.content} />
      </div>
    )
